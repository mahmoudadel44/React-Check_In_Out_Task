...
import { addToCheckOutCart,removeFromCheckIn} from '../actions/cartActions'
...
{ 
  return(<div className="allData" key={item.resource.id}>
         ...
         />
          ...
          <div><span><Link className="dropdown-item" onClick={()=>this.props.removeFromCheckIn(this.props.checkInItems,item)}><i className="fa fa-trash-alt mr-3"></i><span>Delete</span></Link></span></div>
          </div>
                        </div> 
                           </div>  
                            )})
...
export default connect (mapStateToProps,{ addToCheckOutCart,removeFromCheckIn})(CheckInCard);